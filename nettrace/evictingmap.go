...
//   - In-memory mode (no WithBatchMirror):
...
//   - Streaming/mirror mode (WithBatchMirror enabled):
//     The HTTP client emits batches via a callback; this map is set to
//     avoid doing its own flushes.
//     Any persistence (e.g., to BoltDB) is handled by the callback.
