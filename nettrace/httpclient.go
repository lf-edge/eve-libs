...
// WithBatchMirror option is used to enable batch mirroring. Use in NewHTTPClient(..., &WithBatchMirror{...}).
type WithBatchMirror struct {
	Callback          BatchCallback
	Threshold         int  // per evicting map; default 100 if <= 0
	FinalFlushOnClose bool // emit leftovers on Close()
}

func (*WithBatchMirror) isTraceOpt() {}

// If your opts framework checks for defaults, this makes it compatible.
func (o *WithBatchMirror) setDefaults() {
	if o.Threshold <= 0 {
		o.Threshold = defaultBatchThreshold
	}
}
...
type HTTPClient struct {
	...
	*http.Client
	...
	// Batch mirror (count-based, no timers)
...
func (c *HTTPClient) resetTraces(delOpenConns bool) error {
	...
	// If no callback configured, default to a safe cap to avoid unbounded growth.
	capacity := c.batchThreshold
	if capacity <= 0 {
		capacity = defaultCap
	}
	// Ensure we have a sane per-map threshold (used by our own flusher; not by the maps).
	if c.batchThreshold == 0 {
		c.batchThreshold = defaultBatchThreshold
	}

	// We never want evictingMap to flush by itself in this design.
	// - In mirror mode, we flush via flushIfThresholdLocked.
	...
	// Process anything queued up to now.
...
func (c *HTTPClient) GetTrace(description string) (HTTPTrace, []PacketCapture, error) {
	...
	// If we are in batch-mirror mode, do NOT aggregate in-memory maps.
	...
	// Build connection traces (TCP and UDP)
	if len(c.connections.order) > 0 {
		ids := append([]TraceID(nil), c.connections.order...)
		for _, id := range ids {
			if v, ok := c.connections.Get(id); ok {
				if cn, ok := v.(*connection); ok {
					conntrack := conntrackToExportedEntry(cn.Conntrack.flow, cn.Conntrack.capturedAt)
					switch cn.proto {
					case syscall.IPPROTO_TCP:
						tcp := TCPConnTrace{
							TraceID:          cn.ID,
							FromDial:         cn.DialID,
							FromResolver:     cn.FromResolver,
							HandshakeBeginAt: cn.SockCreatedAt,
							HandshakeEndAt:   cn.ConnectedAt,
							Connected:        !cn.ConnectedAt.Undefined(),
							ConnCloseAt:      cn.ClosedAt,
							AddrTuple:        cn.addrTuple.toExportedAddrTuple(),
							Reused:           cn.Reused,
							TotalSentBytes:   cn.TotalSentBytes,
							TotalRecvBytes:   cn.TotalRecvBytes,
							Conntract:        conntrack,
						}
						if c.withSockTrace {
							tcp.SocketTrace = &SocketTrace{SocketOps: cn.SocketOps}
						}
						httpTrace.TCPConns = append(httpTrace.TCPConns, tcp)
					case syscall.IPPROTO_UDP:
						udp := UDPConnTrace{
							TraceID:        cn.ID,
							FromDial:       cn.DialID,
							FromResolver:   cn.FromResolver,
							SocketCreateAt: cn.SockCreatedAt,
							ConnCloseAt:    cn.ClosedAt,
							AddrTuple:      cn.addrTuple.toExportedAddrTuple(),
							TotalSentBytes: cn.TotalSentBytes,
							TotalRecvBytes: cn.TotalRecvBytes,
						...
						if c.withSockTrace {
							udp.SocketTrace = &SocketTrace{SocketOps: cn.SocketOps}
						}
						httpTrace.UDPConns = append(httpTrace.UDPConns, udp)
					}
				}
			}
			c.connections.Delete(id)
	...
	if len(c.dnsQueries.order) > 0 {
		ids := append([]TraceID(nil), c.dnsQueries.order...)
		for _, id := range ids {
			if v, ok := c.dnsQueries.Get(id); ok {
				if q, ok := v.(*DNSQueryTrace); ok {
					// Best-effort backfill FromDial from connection if missing.
					if q.FromDial.Undefined() {
						if cv, ok2 := c.connections.Get(q.Connection); ok2 {
							if cn, ok2 := cv.(*connection); ok2 {
			...
			if cn, ok := v.(*connection); ok {
				...
				case syscall.IPPROTO_TCP:
					tcp := TCPConnTrace{
						...
						TotalSentBytes:   cn.TotalSentBytes,
						TotalRecvBytes:   cn.TotalRecvBytes,
						Conntract:        conntrackToExportedEntry(cn.Conntrack.flow, cn.Conntrack.capturedAt),
					}
					if c.withSockTrace {
						tcp.SocketTrace = &SocketTrace{SocketOps: cn.SocketOps}
					}
					snap.TCPConns = append(snap.TCPConns, tcp)
					added = true
				case syscall.IPPROTO_UDP:
					udp := UDPConnTrace{
						TraceID:        cn.ID,
					...
					snap.UDPConns = append(snap.UDPConns, udp)
					added = true
				}
			}
		})
...
func (c *HTTPClient) finalizeNoConnSocket(sock *inetSocket, connected bool, now Timestamp) {
	...
	// NEW: enqueue into pending TCP/UDP lists when not connected
	if !connected {
		ct := conntrackToExportedEntry(sock.conntrack.flow, sock.conntrack.capturedAt)
		switch sock.proto {
		case syscall.IPPROTO_TCP:
			c.pendingNoConnTCP = append(c.pendingNoConnTCP, TCPConnTrace{
				TraceID:          IDGenerator(),
		...
		case syscall.IPPROTO_UDP:
			c.pendingNoConnUDP = append(c.pendingNoConnUDP, UDPConnTrace{
				TraceID:        IDGenerator(),
				FromDial:       sock.fromDial,
				FromResolver:   !sock.fromResolvDial.Undefined(),
